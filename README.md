...
| 実際の建物 | 有名建築 11 件のプリセット、または現実の寸法（メートル）とスケールから |

- 姫路城・タージ・マハル・ギザの大ピラミッド・エッフェル塔など、**有名建築を実寸のスケールで図面化**
- **完成イメージ**。積み上がった姿を 3D で確認。ドラッグで自由に回せて、真上からも下からも見られる
- **湧き潰し**。モブが湧けるマスが全部、光レベル 1 以上になるように灯りを置く
  （距離ではなく、実際と同じ光の広がり方で計算）。欄干や軒下ランタンなどの装飾も足せる
