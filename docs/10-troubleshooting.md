...
Linux / WSL なら、ツールが使うメモリの上限を決められます。

```bash
export CLAUDE_CODE_TOOL_MEMORY_LIMIT=4G   # Bash / PowerShell ツールのメモリ上限
```
...
**`opusplan` が使い勝手の良い折衷案です。** プランモードでは Opus で計画を立て、実行時は自動で Sonnet に切り替わります。計画の質を落とさずに実装コストを抑えられます。
...
上限に達すると、**リセット後に中断したターンが自動で再開されます。** `/config` の **Continue automatically at usage limit** で無効化できます。
デスクトップアプリではセッション上限のカードに **Auto-continue when limits reset** が表示され、チェックすると再開時刻が示されます（週次上限のカードには表示されません）。
...
- 最近の変更を追う → [14 新機能まとめ](14-whats-new.md)
